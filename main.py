...
class ReferralClient:
    def __init__(self, proxy_manager: Optional[ProxyManager] = None, use_real_emails: bool = False):
        self.faker = Faker()
        self.proxy_manager = proxy_manager
        self.use_real_emails = use_real_emails
    ...
    async def process_referral(self, ref_code: str, captcha_service, real_email: Optional[str] = None) -> Optional[Dict]:
        for attempt in range(1, self.max_retries + 1):
            try:
                log_step(f"\nStarting referral process (attempt {attempt}/{self.max_retries})...", "info")
                self._start_new_proxy_session()
                username, email, password = self._generate_credentials(real_email)
...
async def main():
    ...
    print(f"\n{Fore.YELLOW}请选择注册方式:{Style.RESET_ALL}")
    print(f"1. 使用随机生成的邮箱")
    print(f"2. 使用真实邮箱列表（从 emails.txt 读取）")
    email_choice = input(f"{Fore.GREEN}请选择 (1-2): {Style.RESET_ALL}")
    
    use_real_emails = email_choice == "2"
    real_emails = []
    ...
    if use_real_emails:
        try:
            ...
            if not real_emails:
                ...
                num_referrals = len(real_emails)
        except FileNotFoundError:
            log_step("找不到 emails.txt 文件，将使用随机生成的邮箱", "warning")
            use_real_emails = False
    
    if not use_real_emails:
        num_referrals = int(input(f"{Fore.GREEN}请输入你想要邀请的人数: {Style.RESET_ALL}"))

    print(f"{Fore.YELLOW}该机器人支持的验证码服务平台:{Style.RESET_ALL}")
    print(f"1. Capmonster")
    print(f"2. Anticaptcha")
    print(f"3. 2Captcha{Style.RESET_ALL}")
    service_choice = input(f"{Fore.GREEN}请选择你使用的验证码服务平台 (1-3): {Style.RESET_ALL}")
    api_key = input(f"{Fore.GREEN}请输入验证码服务平台的API Key: {Style.RESET_ALL}")

    use_proxies = input(f"{Fore.GREEN}是否使用 Proxy？(yes/no): {Style.RESET_ALL}").lower() == 'yes'
    proxy_manager = None
    ...
    client = ReferralClient(proxy_manager, use_real_emails)
    ...
    for i in range(num_referrals):
        ...
        real_email = real_emails[i] if use_real_emails else None
        result = await client.process_referral(ref_code, captcha_service, real_email)
        if result:
